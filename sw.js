...
const endget = async (path) => {
    ...
    if (info.dev) return await (await lfetch(end)).text()
    //缓存
                    ...
                    case 'bind_hexo':
                        ...
                        switch (q('step')) {
                            ...
                            case 'auth_user':
                                ...
                                res = await (await fetch(end)).json()
                                if (!res.ok) return Response.redirect('/dash?page=install&type=bind_hexo&step=auth_user&error=1')
